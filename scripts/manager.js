function Manager(size = 4,aim = 2048) {
  ...
  this.listener = new Listener({
    ...
    //引用listener的监听键盘响应返回方向driection
    start: function() {
      self.start();
    }
    //引用listener的监听button的按下事件，按下后触发start
  });
  this.start();
  //进入网页后的首次触发start来初始化方格
};

Manager.prototype.start = function() {
  this.score = 0;//初始化分数为0
  this.status = 'DOING';//初始化状态为doing
...
Manager.prototype.addRandomTile = function() {
  //引用grid类中的randomAvailableCell来寻找随机初始位置
  const position = this.grid.randomAvailableCell();
  if (position) {
    // 90%概率为2，10%为4
    const value = Math.random() < 0.9 ? 2 : 4;
    // 随机一个方格的位置，这里删除了重复定义position
...
// 移动核心逻辑，listenerFn由listener监听后触发
Manager.prototype.listenerFn = function(direction) {
  // 定义一个变量，判断是否引起移动
  let moved = false;
  //由getpath得到rowPath与columnPath的移动方向的数组，例如（3,2,1,0）
