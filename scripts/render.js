...
Render.prototype.render = function(grid,{score,status}){
    //清空所有方格的UI实现
    this.empty();
    //将score传入，在start应用时score为0，在listenerFn移动后this.score += merged.value，为累加值
    this.renderScore(score);
    //将状态传入，在start应用时status为DOING，在listenerFn移动后score等于aim时为WIN，在判断失败后为FAILURE
...
/**
 * 渲染单个图块。
 * @param {Object} tile - 图块对象，包含图块的值及其位置。
 */
